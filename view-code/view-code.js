...
KISSY.add('util/view-code', function(S) {
     ...
     * @param {String} [codeBoxStylse = 'width:95%;overflow:auto;padding-left:5px;line-height:1.5em;border:none;border-left:4px solid #b3d465;'] 源码框的style属性或者class名
     */
    function viewCode(triggerCls,codeBoxStyle){
        
        //参数处理
        triggerCls = triggerCls || 'S_ViewCode';
        codeBoxStyle = codeBoxStyle || 'width:95%;overflow:auto;padding-left:5px;line-height:1.5em;border:none;border-left:4px solid #b3d465;';
                ...
                if(D.hasClass(t,triggerCls)){
                    ...
                    if(!t['data-code-box']){
                        ...
                        if(codeBoxStyle.indexOf(':')>-1){
                            var codeBox = D.create('<textarea style="display:none;height:'+lines+'em;'+codeBoxStyle+'"></textarea>');
                        }else{
                            var codeBox = D.create('<textarea class="'+codeBoxStyle+'" style="display:none;height:'+lines+'em"></textarea>');
                        }
